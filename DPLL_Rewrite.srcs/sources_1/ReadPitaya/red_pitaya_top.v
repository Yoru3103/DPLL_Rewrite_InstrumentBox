...
red_pitaya_ps i_ps (
  ...
  // AXI masters
  // GP0 and its register-bus slaves share the same buffered 125 MHz clock.
  .axi0_clk_i(adc_clk),
...
//IBUFDS r_clk (.I (exp_p_io[1]), .IB (exp_n_io[1]), .O (ref_10m_clk_in));
BUFG r_clk    (.O (ref_10m_clk_in   ), .I (exp_p_io[1]   ));
//
...
clk_wiz_0 adc_pll(
    .reset(1'b0),
    .clk_in1(ref_10m_clk_in),
    .clk_out1(apll_adc_125mhz_out),
    .locked()
);
...
ODDR i_adc_clk_p ( .Q(adc_clk_o[0]), .D1(1'b1), .D2(1'b0), .C(apll_adc_125mhz_out), .CE(1'b1), .R(1'b0), .S(1'b0));
ODDR i_adc_clk_n ( .Q(adc_clk_o[1]), .D1(1'b0), .D2(1'b1), .C(apll_adc_125mhz_out), .CE(1'b1), .R(1'b0), .S(1'b0));
...
IOBUF i_iobufp (.O(exp_p_in[8-1:2]), .IO(exp_p_io[8-1:2]), .I(exp_p_out_hk[8-1:2]), .T(~exp_p_dir[8-1:2]) );
//IOBUF i_iobufn (.O(exp_n_in[8-1:2]), .IO(exp_n_io[8-1:2]), .I(exp_n_out_hk[8-1:2]), .T(~exp_n_dir[8-1:2]) );
IOBUF i_iobufp2  (.O(exp_p_in[0]), .IO(exp_p_io[0]), .I(exp_p_out_hk[0]), .T(~exp_p_dir[0]) );
//IOBUF i_iobufn2  (.O(exp_n_in[0]), .IO(exp_n_io[0]), .I(exp_n_out_hk[0]), .T(~exp_n_dir[0]) );
