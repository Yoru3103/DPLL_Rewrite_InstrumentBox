...
#### 8.2.1 阶段 2 首轮候选表（待实物确认）

阶段 2 的目的不是直接冻结最终参数，而是用一组受限、可回退的初始候选验证参数扫描流程。运行时首先保存当前参数为 `ORIGINAL`；`ORIGINAL` 参与基线评分并始终作为最终候选和失败回退值。下表中的数值只用于首轮实物筛选，实验后必须依据日志修改本文档，再同步修改代码。

测频回路候选表：

| ID | profile | Kp | Ki | Kii | Kd | D_COEF | 首轮用途 |
|---:|---|---:|---:|---:|---:|---:|---|
| 0 | ORIGINAL | 运行时读取 | 运行时读取 | 运行时读取 | 运行时读取 | 运行时读取 | 基线、最终保底和回退 |
| 1 | SAFE | `0x00200000` | `0x00040000` | 0 | 0 | `0x0000FFFF` | 低增益安全候选 |
| 2 | TRACK_WEAK | `0x00200000` | `0x00080000` | 0 | 0 | `0x0000FFFF` | 弱信号低带宽候选 |
| 3 | TRACK_MEDIUM | `0x00300000` | `0x000C0000` | 0 | 0 | `0x0000FFFF` | 中等幅值候选 |
| 4 | TRACK_STRONG | `0x00400000` | `0x00100000` | 0 | 0 | `0x0000FFFF` | 当前 Kp/Ki 对应候选 |
| 5 | ACQUIRE | `0x00500000` | `0x00100000` | 0 | 0 | `0x0000FFFF` | 仅用于后续频率阶跃实验，默认扫描不启用 |
...
|---:|---|---:|---:|---:|---:|---:|---|
| 0 | ORIGINAL | 运行时读取 | 运行时读取 | 运行时读取 | 运行时读取 | 运行时读取 | 基线、最终保底和回退 |
| 1 | SAFE | 550000 | 250000 | 0 | 0 | `0x0000FFFF` | 低增益安全候选 |
| 2 | TRACK_WEAK | 550000 | 500000 | 0 | 0 | `0x0000FFFF` | 弱信号低带宽候选 |
| 3 | TRACK_MEDIUM | 825000 | 750000 | 0 | 0 | `0x0000FFFF` | 中等幅值候选 |
| 4 | TRACK_STRONG | 1100000 | 1000000 | 0 | 0 | `0x0000FFFF` | 当前 Kp/Ki 对应候选 |
| 5 | ACQUIRE | 1375000 | 1000000 | 0 | 0 | `0x0000FFFF` | 仅用于后续频率阶跃实验，默认扫描不启用 |

首轮默认扫描顺序为 `ORIGINAL → SAFE → TRACK_WEAK → TRACK_MEDIUM → TRACK_STRONG`，不自动测试 ACQUIRE。这样每个候选都位于当前 Kp/Ki 的 25%～100%积分增益和 50%～100%比例增益范围内，不在首轮测试中提高闭环增益。ACQUIRE 必须由专门的阶跃测试模式显式启用。

候选表执行规则：

1. 代码只接受表内完整参数组，不接收在线自由搜索生成的系数。
2. `ORIGINAL` 永不被候选表覆盖；CANCEL、全部候选失败、读回错误或最终验证失败均恢复 ORIGINAL。
3. 首轮候选固定 `Kii=0、Kd=0`。这会与锁相回路当前 Kii/Kd 不同，因此每次测试都必须经过受控解锁、完整写入、读回、重锁和 SETTLE；若首个候选无法恢复锁定，立即回退，不继续扫描更激进候选。
4. ORIGINAL 的当前 Kii/Kd 保持原样，只在候选 profile 中关闭这两项，确保回退能够恢复原系统行为。
5. 最佳候选必须优于 ORIGINAL，且最终 VERIFY 通过；否则仍选择 ORIGINAL，不把“完成扫描”等同于“必须换参数”。

#### 8.2.2 阶段 2 采样窗口和硬约束

方案 B 阶段 2 使用以下冻结初值，实物测试后允许通过先修订本文档再调整：

| 项目 | 测频回路 | 锁相回路 |
|---|---:|---:|
| 活跃采样周期 | 1 ms | 1 ms |
| 候选 SETTLE | 300 ms | 800 ms |
| BASELINE/EVALUATE | 1000 ms | 1000 ms |
| 最终 VERIFY | 1000 ms | 1000 ms |
| 最低有效样本数 | 800 | 800 |
| 最低锁定样本比例 | 90% | 90% |
| 允许稳定限幅样本数 | 0 | 0 |
| 允许稳定残差越限比例 | 5% | 5% |

每个采样点读取幅值、14 bit 瞬时频差、32 bit 相位残差、限幅后输出和低 6 bit 状态。累计幅值均值/极值、频差绝对均值/峰值、相位残差绝对均值/峰值、输出范围、锁定样本数、限幅样本数和残差越限样本数。

阶段 2 代价函数使用整数运算：

```text
unlock_permille   = 1000 * unlocked_count / sample_count
rail_permille     = 1000 * rail_count / sample_count
residual_permille = 1000 * residual_bad_count / sample_count

J = avg_abs_freq
  + (avg_abs_phase >> 10)
  + 1000 * unlock_permille
  + 4000 * rail_permille
  + 2000 * residual_permille
```

评分以 32 bit 保存并通过扩展遥测读取；旧 16 bit 分数字段返回饱和值以保持阶段 1 协议兼容。出现以下任一情况时不再比较分数，直接判定候选不合格：

- 样本数不足。
- 锁定比例低于 90%。
- 出现任何稳定正/负限幅样本。
- 稳定频差或相位残差越限比例超过 5%。
- 候选期间幅值均值低于 ORIGINAL 基线幅值均值的 50%。
- 参数读回不一致、取消请求或目标回路无法重新使能。

由于现有协议的 18 byte QUERY 回复没有空间容纳全部原始统计，阶段 2 先保持该回复格式不变；`currentScore/bestScore` 用于快速观察。完整实验数据导出命令在获得首轮实物数据后再定义，定义前必须先修订本文档。
...
阶段 2 同步提供 `host_app` Python/PyQt6 上位机，使用现有 UART 协议，不为手动控制重复定义命令。软件分为串口协议层、串口工作线程、手动控制页、自调监控页、批次验证执行器和日志层：

- 串口工作线程独占串口并串行执行请求，GUI 线程不得阻塞等待响应。
- 手动控制覆盖两路中心频率和 Kp/Ki/Kii/Kd 完整读写、DPLL 开关与测频复位。
- 自调监控覆盖 0x97/0x98 的 QUERY/START/CANCEL/CLEAR/SET_POLICY，并显示 RUN_ID、进度、状态、profile 和评分。
- 自动查询默认周期为 250 ms，固件仍以内部 1 ms 周期采样；上位机查询频率不能替代固件采样频率。
- 批次验证使用 JSON 计划。每个案例可选择保持当前配置，或先写中心频率和完整 PID，再串行执行指定次数的 autotune。
- 每次查询和最终结果同时保存为 UTF-8 CSV 与 JSONL，日志包含案例名、目标回路、重复序号、RUN_ID、状态、profile、评分、耗时和实验备注。
- 批次验证不得并行启动两路；停止或超时时先发送 CANCEL，后续案例仍受固件全局仲裁器保护。
- 上位机显示和写入的是 PL 原始寄存器值；在换算公式冻结前不擅自显示为 Hz 或浮点增益。

批次验证的推荐顺序为：通信与回退测试、固定输入重复性测试、强/中/弱幅值分档、频率阶跃/扫频/掉信号测试、汇总日志并将最终门限和参数回写本文档。
...
状态：**PS 软件实现和 SDK 构建已完成**。实现提交为 `bd7a1a8`，注释评审提交为 `a87ea5c`；板卡 UART 实物验收并入阶段 2 首轮测试，不据此重复验证阶段 0。
...
- 首轮实现和测试严格使用第 8.2.1、8.2.2 节冻结的候选表、扫描顺序、时间窗口和硬约束；实物数据要求调整任何数值时，先修订本文档。

完成条件：证明两条回路在不同信号条件下都存在可重复的最佳参数档，并确定门限、稳定等待时间、评分窗口和回退边界。

当前状态：**候选参数已通过用户评审；PS 候选扫描和 Python 上位机已实现并通过离线构建/单元测试，等待板卡 UART 实物测试**。
...
8. 是否确认阶段 2 首轮候选使用第 8.2.1 节数值，并在候选中关闭 Kii/Kd、在 ORIGINAL 中保留其运行时原值。
9. 方案 C 允许的 PL 资源增量。
